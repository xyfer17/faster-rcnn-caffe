...
namespace caffe {
	...
	void RPNLayer<Dtype>::Forward_cpu(
		...
		for (int i = 0; i < height; ++i)
		{
		 ...
		 //get sores to the right, the anchors_nums_ one-point bg score in the front, and the anchors_nums_ the fg score in the back, what we need is the latter.
		...
		src_scale_ = bottom[2]->data_at(0, 2, 0, 0);
		 ...
		 //gen local anchors to the right
		...
		//clock_t start, end;
		//start = clock();
		 std::sort(aboxes.rbegin(), aboxes.rend()); //descending order
		if (pre_nms_topN_ > 0)
		{
